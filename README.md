...
## Note
As of version 0.5.0-beta, Hermeris has an additional `/cmd` feature as an alternative to prefix commands for certain simpler features. A list of those text-based commands are at the end of this document.
...
Hermeris uses the py-enigma library to provide a virtual Enigma Machine for text manipulation. Please refer to the manual entry on the [/enigma](#/enigma) command for help with the syntax.
...
#### Purpose:
   ...
   ```
   /anagrams rotors:II IV V, reflector:B, ring_settings:1 20 11, plugboard_settings:AV BS CG DL FU HZ IN KM OW RX, starting_positions:WXC
   ```
   (Plain text has not been shown)
<hr>
